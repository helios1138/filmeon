...
This is a scene-referred, color-managed approach to inverting color-negative film scans, adapted for use in DaVinci Resolve. By grounding the math in the physics of how both film and scanners work, we aim to achieve consistent results (accurate tone response and pleasing colors) from film negatives.

The process is loosely inspired by Hollywood pipelines for scanning motion picture film, such as the old Kodak [Cineon System](https://en.wikipedia.org/wiki/Cineon) and the modern [ACES](https://en.wikipedia.org/wiki/Academy_Color_Encoding_System) workflow (ADX/APD), but with greater flexibility to accommodate uncalibrated home/consumer scanning. We don't have to use color targets or exposure ramp film strips (though, of course, we can if we need to).

First, a little bit about the theory behind it.

## The theory behind it

The problem with a more "naive" approach, which you might have seen elsewhere (when a negative scan is inverted and then curve-fitted directly into an SDR output), is that it mixes together the properties of the film, the scanner, and the scene together in a way that makes the look of the final image somewhat arbitrary and the process is inconsistent.

Instead, we first convert the negative to a roughly linear representation of the photographed scene, then tone-map it to the desired output, whether SDR or HDR. That way, we can perform color interpretation, white balance, and exposure adjustments independently, and correct scanner deficiencies in a consistent, repeatable way.
...
Instead, the best way to understand the tone response of color negative film is to look at the characteristic curves (also known as Hurter–Driffield curves or H-D curves). These curves lie in logarithmic space, plotting the logarithm of exposure against the optical density (logarithm of opacity) of the developed film.
...
_The Cineon System, among other things, defined a specific scale for representing scanned density values in 10-bit files for storage and interchange (minimum density starts at 95/1023, with every next code value increase representing 0.002 units of density). We will use this scale for our demo._
...
On the following chart, you can see this visualized: the thin green line representing the ideal $\gamma=1.0$ response, in relation to our film's actual (now aligned) curves. Depending on how the film was developed (time/temperature affects this) and scanned (more on this later), this gamma can vary slightly, but in a good scan, it usually sits around 0.6.

<img src="./img/d-unbuild-gamma.png" width="400" />

### Converting to scene-linear exposure values and tone-mapping

We now know that we need to match the film's gamma $\gamma$ to re-scale the curve, but we also need a reference/anchor point to match the shot's exposure level. Our approach offers two formulas for this, each with a slightly different philosophy.

The first is a classic Cineon formula that anchors the image between black and white points. You choose the gamma (defaults to $\gamma=0.6$) and the white point (defaults to $D=1.18$ above $D_{min}$), which maps to a linear exposure value of $E=1.0$, and the $D=0.0$ is mathematically forced to land on $E=0.0$ (using anti-log black offset). The rest of the exposure range (super-whites) falls above $E=1.0$. Because of this black offset, we do not need to (in fact, we should not) linearize the toe. This approach gives you true black point out of the box, but is less physically accurate, as it ignores the actual shape of the scene shadows.

_This is also the formula used in DaVinci Resolve's built-in (CST) Cineon Log-to-Linear transform._
...
This is perhaps the most important part. As already mentioned, the look you get from an inverted negative scan is often quite arbitrary if we don't account for the factors that influence it. Specifically, if we just treat the RGB channels of a scan as in any way equivalent to either the scene or the display primaries. More often than not, what you get does not represent the native film look in any meaningful way (even if it might accidentally turn out acceptable or pleasing).

$$
RGB_{scene\ capture} \neq RGB_{film\ dyes} \neq RGB_{scan} \neq RGB_{display}
$$

Well, what is a "film look" anyway? Why do some people say "X film has better skin tones, while Y film has better greens, but renders the skin that's too pink"?

The truth is that color negative film was designed to be only half of the complete photographic system; the other half was the well-defined printing process using a corresponding type of printing paper (or print film). Only together did they produce the desired color rendition. If we want to call something the true "film look", this would be it.
...
- The spectral absorption characteristics of the dyes formed during development (the dyes are not perfect absorbers, e.g., no such magenta dye can be made such that it would only block green light and pass 100% of red and blue)
- The spectral emission characteristics of the light that shines through the film during printing (or scanning)

The paper's own characteristics and the whole printing process were specifically engineered/tuned to account for and complement the above, producing the desired result. And obviously, no major film manufacturer has ever intentionally designed a process that would render skin "too pink". Thus, if in your digital scan you're seeing significant color casts in shadows or the highlights or dramatically shifted hues in, e.g., skin tones or foliage, it's more likely that you're looking at the properties of your scanning and inversion process, rather than the film itself.

Essentially, our goal in this process is to be able to define our own "digital printing" color response intentionally and in a way that is consistent and repeatable (across different scans of the same film on the same scanner).

For this purpose, we add two user-controlled 3x3 color matrices into our workflow. One is applied in density space to control dye mixing/unmixing (accounting for different abovementioned kinds of crosstalk), and the other is applied in scene-linear exposure space to map colors into a known display-referred color space.

_Technically, unless you're using a monochromatic sensor, we'd need a third matrix to account for sensor crosstalk (applied in linear transmittance space), but in practice, it's a bit of an overkill for a manual workflow like ours._

The density crosstalk matrix starts as an identity matrix (the original mix), and the adjustment direction depends on your scanner setup. If you're working with a lab-like trichromatic scan (combined exposed under narrowband red, green, and blue light), where emission peaks of the lights are tuned to match the dye absorption peaks, you will measure density that is not equal, but very close to the actual dye amounts in the emulsion (a.k.a. analytical density). Then, you would want to mix the channels slightly to better align with the original scene capture, since we're targeting a wide color gamut and working against the development chemistry (DIR couplers). If your scan uses broadband (white) light, the density measurements will already be mixed up quite a bit and not in the way we want. In this case, you would want to un-mix (separate) them instead. In practice, we'll likely end up doing both.

It might have sounded more complicated than it is; in practice, you're just visually adjusting color separation, aiming for a happy medium between compressed, neon-like colors on one end and overmixed, dull colors on the other.

_In addition to getting mixed density values, a spectrally leaky scanner setup will result in too much light being recorded across all channels, lowering the overall measured density values, and thus, default gamma and exposure anchor assumptions used in professional lab scanning are likely not going to be good for us. That's one of the reasons we're using adjustable parameters in our density-to-scene linear formulas._
...
_Reminder: we're looking at the density channels, not final colors. We're treating them as sRGB/Rec.709 colors for visualization. Also, the shots had their scale (gamma) adjusted to match, so we'd only focus on the color separation differences:_
...
The second (linear exposure) matrix starts as a reference transform based on the one used in ACES film workflows (just chromatically adapted to the D65 white point used in our workflow). It should provide a good enough starting point (and a fixed anchor under which the previous matrix initial adjustments are best judged), but you can tweak the color vectors to actually lock in the look you want. The control scheme for this one is primary color vector rotation and scaling in CIE chromaticity space; if you've used the Color Calibration (or Camera Calibration) tool in Lightroom before, you'll already be familiar with how this works.

<img src="./img/cie-gif.gif" width="500" />

So, our two matrices together produce a consistent, repeatable custom color response for a given film stock shot under a given illuminant, while (mostly) accounting for the properties (or deficiencies) of your specific scanner setup. Think of this as a custom digital printing paper.
...
The process requires a **linear** and **non-colour-managed** scan as input.

If you're using a dedicated film scanner with SilverFast or VueScan, scan in RAW and export as a TIFF file; you should be good to go. If it's a DNG, make sure you know how it's tagged, because Resolve will interpret it based on that metadata and the settings in the Camera RAW module. For example, my Plustek DNG scans from SilverFast require setting the white balance in Camera RAW to "Cloudy" (6500/10) and the color space to Rec.709 / Linear to make sure the RAW processor outputs the original sensor data as-is (no-op). These settings don't make any physical sense; they just match the DNG metadata exactly. Alternatively, you can just convert the DNG back to a TIFF, as described below. _(Technically, DNGs are already TIFFs just with extra metadata, and dedicated film scanners do not use Bayer filters, so we'd just be stripping the metadata in this case)._

If your scans are DSLR/Mirrorless shots, you'll need to convert the DNG or camera-native RAW files to linear non-colormanaged TIFFs. One way to do this is using the [MakeTiff](https://www.colorperfect.com/MakeTiff/) utility. It will debayer the RAW data, but otherwise will output the original sensor data as-is. This is very different from exporting a TIFF from Lightroom; even when using linear profile, the images will already have Adobe color management applied, which we don't want.

The resulting files from MakeTiff will look greenish, and that's as it should be, because you're seeing the raw channel values before the white-balance scaling. You might think this is because the Bayer filter has more green pixels, but the real reason is that most camera sensors' green pixels are simply more sensitive to light. This is also the reason why you should be very careful if you're using ETTR when scanning the film - the green channel will clip first, and the histogram on your camera will likely lie about where the clipping happens. Clipping the green channel in the thinnest/low-density regions of the negative is a great way to ruin shadow neutrality in a scan. If you want to learn more about this, I'd recommend looking into **UniWB** concept.

And obviously, for the inversion process to produce consistent results and for the settings to be meaningfully reusable, you should lock down your scanning setup as much as possible. The camera, lens, and exposure settings should be locked at least for the entire roll. Light source (both spectrum and intensity) should be the same across all rolls of the same film stock if we want our density remixing matrix to remain valid.

#### The process

Our first step is picking the film base and two neutral colors from the reference shot. Unfortunately, the color pickers on the Color page in Resolve can only sample a single pixel, and the values are quantized to 8 bits (regardless of what the tooltip will show). This is not good enough for our purposes, so we need to make some compromises:

Compromise 1: Do the entire work in the Fusion page. Fusion color pickers pick raw/precise floating point values, can pick values <1 or >0, and can pick an average from an area. The downside is that it's just not that convenient to work in Fusion.

Compromise 2: Pick the colors in Fusion, then transfer them to the Color page. We'll have to use some trickery for this. In the FilmeonScan DCTL, this is what **Write Ref**/**Read Ref** checkboxes are for. When **Write Ref** is checked, the node skips all subsequent calculations and writes the selected colors directly to the image as the first 3 pixels. Then on the Color page, **Read Ref** reads those pixels instead of using the color pickers. For this to work, either the timeline resolution must exactly match the scan, and you cannot rotate or crop, or you need to create a New Fusion Clip from a resized/rotated/cropped version and work on that. Then you can copy and paste the Fusion grade across all clips (shots from the same roll), and they will all use the same reference colors. The downside here is just all the complexity, though this is the most precise way to do it.

Compromise 3: This is the easiest way and one we'll focus on. The powergrade includes additional nodes for this purpose. We use the Blur node to compensate for the lack of area-averaging, and we pick the film base using the **Pick White Point** of the **Gain** primary color wheel in the previous node instead. The Scan node then receives the channels that have already been pre-scaled. That way, we can minimize the impact of the 8-bit quantization when picking the two neutral points (because the ranges are now bigger). If we want/need to perform Glare correction, we'll need to manually copy the Gain coefficients to the Scan node so it can compute the original scanneed values internally (because Glare correction only makes sense relative to the unaltered scanner output).

So, assuming we go with Compromise 3, let's run through the nodes in the Filmeon powergrade:

- **Zoom** (Transform)
  - Can be used for temporarily zooming in to crop out the film/holder frames so that we can see the **extents** on the parade/waveform scope to help with adjusting the film base offset and highlight clipping. It can also be used for actual framing/cropping, but I prefer to do this outside Resolve, in software better suited to image processing.
- **Blur** (Gaussian Blur)
...
- **Scan** (FilmeonScan DCTL)
  ...
  - **Balance Mode** switches between whether we're using two neutral points to align the channels or a single point (if both selected, the brightest). If only one point is selected, single-point balancing is used in both cases. Two are better, but one is better than none, if you don't have the anchors. Note, though, that single-point balancing uses only gains/scaling, not offsets, so it's less effective the farther your scene is from the native white balance of your film stock.
...
- **Lin** (FilmeonConvert DCTL)
  ...
  - The modes we're interested in are **D to E** (classic Cineon) and **D to E (Pines)**, which convert the density to scene-linear exposure
  ...
  - Other modes include the reverse conversion (from scene linear to density, all other controls also become inverted) and the basic density-to-transmittance and transmittance-to-density conversion (all other controls except Cineon Scale do not function in these modes).
...
- **CA** (Chromatic Adaptation)
  ...
  - You set the **target illuminant** (temperature/tint) to the lighting of the **reference shot** (the one we used for adjusting the color). Obviously, this is nothing more than an educated guess (unless it's known), but you can get pretty close if you know what lit the scene. Midday vs. sunset, sunny vs. cloudy, etc., give you the temperature. Daylight should be fairly neutral, whereas LED lights often show a noticeable green tint. _Importantly, this has nothing to do with the native balance of the film stock (daylight/tungsten)._
  - Then you can play around with the **source illuminant** sliders to correct for the actual lighting of the **current shot**. If the numbers no longer make sense, you can go back and re-estimate the target.
- **ODT** (Color Space Transform)
  ...
  - SDR images are encoded using **sRGB/sRGB**. Alternatively, you can use a wide-gamut **P3-D65/sRGB** target (also known as Apple's **Display P3** or Adobe's **image P3**). Resolve is not capable of tagging (assigning an ICC profile) image outputs, so sRGB/sRGB is the simplest option, since most other software assumes sRGB by default for images without a profile. Later, we'll go over how to assign the profile in Photoshop (which is also required for HDR output).
  ...
  - The tone-mapping method (mainly, I consider **DaVinci** vs **Luminance Mapping**) will also affect the final colors of your SDR output. DaVinci aims for perceptual hue mapping, and Luminance Mapping aims for more colorimetrically accurate hue preservation. The difference is especially stark when dealing with yellow-orange axis highlights (fire, sunset, brightly lit skin tones, etc.).
  - **Apply Forward OOTF** - applies the reference transform from scene-referred to display-referred spaces. In practice, for our purposes, this works like having a print/output "toe", which improves shadow rendering. Historically, it derives from the old SDR video standard (and was subsequently explicitly codified in the current HDR standard). You can, of course, just use your own custom tone curve instead.
...
For the sake of completeness, here are the resulting photos from the process shown in the video, both SDR and HDR versions (obviously, the difference is only visible on an HDR display).
...
_Tip: For dealing with different orientations, I would recommend just setting the timeline resolution to a square, with a side = max(width, height) of your scans, and dealing with actual cropping or other transforms in a software better designed for working with images._
...
To render the most compatible HDR output, I use Lightroom. JPEG (with gain map) is probably the best, since you have control over HDR > SDR tonemapping and the resulting image looks good (and the way you want it), both for people with or without an HDR display. Alternatively, AVIF is the best HDR-native format, with PQ encoding and actual MaxCLL/MaxFALL tagging, but the SDR fallback is out of your hands.

But in order to get the image from Resolve into Lightroom, we need to jump through a few hoops. You could tag a 16-bit TIFF with a PQ-based profile in Photoshop, but Lightroom, unfortunately, does not respect the absolute nature of the PQ curve, does its own interpretation into its own relative HDR paradigm (Lightroom works with "stops above SDR white", rather than display nits), and the image will look a little bit different as a result. The only way to get the image into Lightroom intact is to move the raw floating point linear data from Resolve. Unfortunately, Resolve cannot output floating-point TIFFs, and Lightroom cannot read anything else.

So the hoops we need to jump through are:

- Configure your ODT for the HDR target. Usually, it's P3 or Rec.2020 color with PQ (ST2084) transfer function, and check "Use HDR 203 Nits Diffuse White" if it didn't get checked automatically (if you use a custom tone curve this doesn't matter as much, but out-of-the-box scene-to-HDR will look better this way when looked at in a bright environment, where photos are usually viewed).
...
- After the ODT, add another CST node to convert the graded image back to linear data (e.g., P3/ST2084 to P3/Linear). Tone Mapping set to None; all checkboxes disabled. We need a purely mathematical conversion.
- On Deliver page, choose **EXR** / **RGB float (PIZ)**. You can also use **RGB half (PIZ)** if you're concerned about disk space (these files are huge, even with PIZ compression). Render the images.
- At this point, we need to assign a corresponding linear ICC profile
  ...
  - Open Photoshop. **Edit** > **Color Settings**. In **Working Spaces**, click on **RGB** and select **Custom RGB**
  ...
  - Click OK. It will create the profile and set it as the default RGB working space, which we don't need. Hit **Cancel** to exit Color Settings without changing the default.
...
- Once you have the profile, open the EXR files and assign them to it, then save as 32-bit or 16-bit float TIFF files (ZIP compression will reduce the file size, but it might take much longer to render; worth it if you plan to store them long-term, though). Again, to speed things up, record an Action and use Batch processing.

That's it. Now these files can be imported into Lightroom, and once you enable HDR mode, they should look exactly as they do in Resolve's viewer, and you can export them in a consumer-friendly format from there.
